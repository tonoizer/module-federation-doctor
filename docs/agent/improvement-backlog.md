...
| --- | --- | --- |
| Shared singleton / versions / `import: false` / prefix share | Strong | `packagePath`, `layer`, bundler `externals` ∩ `shared` |
...
still cannot *drive* most of them: there is no `examples/showcase/vite/`
...
| **effort** | M |
| **score** | 67 |
| **why** | Webpack layer sharing is a real Enhanced/webpack failure mode. Not in `NormalizedShared`. **Low confidence** on how often MF users set `layer`; confirm SDK types before a rule. First PR can normalize-and-unknown-field only. |
| **evidence** | `src/types.ts` `NormalizedShared`; CONTRIBUTING SDK plugin types |
