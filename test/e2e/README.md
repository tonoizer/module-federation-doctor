...
1. **Read the assertion message** — each expect includes the remote name and URL
   (for example `rspack remote (http://localhost:3001/remoteEntry.js)`).
...
3. **Confirm which webServer failed** — `playwright.config.ts` starts three
   ...
   `webServer.url` means that preview process never became ready.
...
```bash
curl -fsS http://localhost:3001/remoteEntry.js | head
curl -fsS http://localhost:3002/remoteEntry.js | head
