...
## Gemeinsame und Core/Rspack/Rsbuild-Optionen
...
| `dataPrefetch` / prefetch remotes    | Prefetch/preload misconfig can break load order                                     | Use [`prefetch`](https://module-federation.io/guide/data/data-prefetch.html) after `lazyLoadComponentPlugin` with `<remote>/<expose>` ids; test `preloadRemote` load order. Not checked automatically (no normalize field — low confidence). |
...
| Nested producer DTS                  | Missing extracted remote types in producer archive                                  | Enable `dts.generateTypes.extractRemoteTypes` (`config/nested-producer-dts-extract`)                                                                                                                                                         |
