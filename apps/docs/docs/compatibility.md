...
| **pending**     | In scope for v1, gated on an open adapter issue; other cells are not blocked                    |
...
| Webpack              | **pending**               | — (gated on [#10](https://github.com/tonoizer/module-federation-doctor/issues/10)) | Not claimed until the adapter lands on `main`              | Do **not** treat Webpack as supported while [#10](https://github.com/tonoizer/module-federation-doctor/issues/10) / adapter PR is open. Other cells stay green independently. |
...
1. Vite, Rspack, or Rsbuild real build + Doctor path fails on Node 22 or 24 in
...
1. **Webpack pending** on
   [#10](https://github.com/tonoizer/module-federation-doctor/issues/10) —
   document as pending; do not claim Webpack **supported** until the adapter is
   on `main` with CI evidence.
2. npm/yarn consumer-path differences — documented partial; monorepo CI stays
   pnpm.
3. Expected `doctor/partial-analysis` warnings on partial analysis paths —
   honest gaps, not matrix failures.
4. post-v1 adapters (Rolldown, Modern.js) — out of scope for v1 close.

v1 close still requires Webpack to be **supported** or explicitly **partial**
with the same honesty rules once [#10](https://github.com/tonoizer/module-federation-doctor/issues/10)
lands; this page can ship incrementally with Webpack **pending**.
...
| `package.yml`                 | Pack/consume smoke on Node 22 + 24                              |
