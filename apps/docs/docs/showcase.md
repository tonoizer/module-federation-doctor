...
These directories are demo fixtures, not runnable apps. Prefer fixtures where a
missing or weak config would still reach a Module Federation plugin or build;
hard plugin failures (for example an empty `name`) stay as rules, not showcases.

Keep [mixed federation](./mixed-example.md) for the healthy multi-bundler path.
