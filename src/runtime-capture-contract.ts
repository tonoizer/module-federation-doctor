...
/**
 * Offline runtime-capture contract used by `mfdoctor runtime`.
 *
 * This module is the file-import path: envelope types, quotas, redaction, and
 * validation. It does not attach to a browser, inject into a page, or project
 * live snapshot/network fallbacks. Those unused transports live in
 * `runtime-capture-transports.ts` and are not imported by the CLI.
 */
...
export const SECRET_KEY = /token|secret|password|credential|authorization|cookie|api[-_]?key/i;
...
export function ownDataEntries(
...
export function parseSafeValue(
  value: unknown,
  limits: RuntimeCaptureLimits,
  path: string,
  depth = 0,
...
export function assertLimits(limits: RuntimeCaptureLimits): void {
