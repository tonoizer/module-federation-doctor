...
const doctor = moduleFederationDoctorPlugin({
  ...
  failOn: "never",
  rules: {
    // Cell focuses on adapter emit + shared findings, not DTS generation.
    "artifact/types-missing": "off",
    // Simulated cell uses enhanced/rspack under the hood (not @module-federation/modern-js).
    "config/plugin-package-mismatch": "off",
  },
});
await doctor.setup({
  getAppContext: () => ({
    packageName: "@mfdoctor-standalone/modern",
    command: "build",
    metaName: "standalone-modern",
    bundlerType: "rspack",
    appDirectory: root,
    isProd: true,
  }),
